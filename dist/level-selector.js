(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["LevelSelector"] = factory(require("@clappr/player"));
	else
...
"use strict";
/* WEBPACK VAR INJECTION */(function(process) {Object.defineProperty(exports,"__esModule",{value:true});var _classCallCheck2=__webpack_require__(43);var _classCallCheck3=_interopRequireDefault(_classCallCheck2);var _createClass2=__webpack_require__(44);var _createClass3=_interopRequireDefault(_createClass2);var _possibleConstructorReturn2=__webpack_require__(49);var _possibleConstructorReturn3=_interopRequireDefault(_possibleConstructorReturn2);var _inherits2=__webpack_require__(76);var _inherits3=_interopRequireDefault(_inherits2);var _player=__webpack_require__(84);var _levelSelector=__webpack_require__(85);var _levelSelector2=_interopRequireDefault(_levelSelector);var _style=__webpack_require__(86);var _style2=_interopRequireDefault(_style);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj};}var AUTO=-1;var LevelSelector=function(_UICorePlugin){(0,_inherits3.default)(LevelSelector,_UICorePlugin);function LevelSelector(){(0,_classCallCheck3.default)(this,LevelSelector);return(0,_possibleConstructorReturn3.default)(this,_UICorePlugin.apply(this,arguments));}LevelSelector.prototype.bindEvents=function bindEvents(){this.listenTo(this.core,_player.Events.CORE_READY,this.bindPlaybackEvents);if(_player.Events.CORE_ACTIVE_CONTAINER_CHANGED)this.listenTo(this.core,_player.Events.CORE_ACTIVE_CONTAINER_CHANGED,this.reload);else this.listenTo(this.core.mediaControl,_player.Events.MEDIACONTROL_CONTAINERCHANGED,this.reload);this.listenTo(this.core.mediaControl,_player.Events.MEDIACONTROL_RENDERED,this.render);this.listenTo(this.core.mediaControl,_player.Events.MEDIACONTROL_HIDE,this.hideSelectLevelMenu);};LevelSelector.prototype.bindPlaybackEvents=function bindPlaybackEvents(){if(!this.playback)return;this.listenTo(this.playback,_player.Events.PLAYBACK_LEVELS_AVAILABLE,this.fillLevels);this.listenTo(this.playback,_player.Events.PLAYBACK_LEVEL_SWITCH_START,this.startLevelSwitch);this.listenTo(this.playback,_player.Events.PLAYBACK_LEVEL_SWITCH_END,this.stopLevelSwitch);this.listenTo(this.playback,_player.Events.PLAYBACK_BITRATE,this.activeLevelChanged);var playbackLevelsAvailableWasTriggered=this.playback.levels&&this.playback.levels.length>0;playbackLevelsAvailableWasTriggered&&this.fillLevels(this.playback.levels);};LevelSelector.prototype.reload=function reload(){var _this2=this;this.stopListening();// Ensure it stop listening before rebind events (avoid duplicate events)
...
var hasLevels=!!(this.levels&&this.levels.length>1);return respondsToCurrentLevel&&hasLevels;};LevelSelector.prototype.render=function render(){if(this.shouldRender()){var style=_player.Styler.getStyleFor(_style2.default,{baseUrl:this.core.options.baseUrl});this.$el.html(this.template({'levels':this.levels,'title':this.getTitle()}));this.$el.append(style);this.core.mediaControl.$('.media-control-right-panel').append(this.el);this.updateGUI();}return this;};LevelSelector.prototype.fillLevels=function fillLevels(levels){var initialLevel=arguments.length>1&&arguments[1]!==undefined?arguments[1]:AUTO;if(this.selectedLevelId===undefined)this.selectedLevelId=initialLevel;var onLevelsAvailable=this.core.options&&this.core.options.levelSelectorConfig&&this.core.options.levelSelectorConfig.onLevelsAvailable;if(onLevelsAvailable){if(typeof onLevelsAvailable==='function')levels=onLevelsAvailable(levels.slice());else throw new TypeError('onLevelsAvailable must be a function');}this.levels=levels;this.configureLevelsLabels();this.render();};LevelSelector.prototype.configureLevelsLabels=function configureLevelsLabels(){if(this.core.options.levelSelectorConfig===undefined)return;var labelCallback=this.core.options.levelSelectorConfig.labelCallback;if(labelCallback&&typeof labelCallback!=='function')throw new TypeError('labelCallback must be a function');var hasLabels=this.core.options.levelSelectorConfig.labels;var labels=hasLabels?this.core.options.levelSelectorConfig.labels:{};if(labelCallback||hasLabels){var level=void 0;var label=void 0;for(var levelId in this.levels){level=this.levels[levelId];label=labels[level.id];if(labelCallback)level.label=labelCallback(level,label);else if(label)level.label=label;}}};LevelSelector.prototype.findLevelBy=function findLevelBy(id){var foundLevel=void 0;this.levels.forEach(function(level){if(level.id===id)foundLevel=level;});return foundLevel;};LevelSelector.prototype.onLevelSelect=function onLevelSelect(event){window.playback=this.playback;this.selectedLevelId=parseInt(event.target.dataset.levelSelectorSelect,10);if(this.playback.currentLevel===this.selectedLevelId)return false;if(this.assumeLevelChangesWillWork)this.updateGUI();this.playback.currentLevel=this.selectedLevelId;this.toggleContextMenu();event.stopPropagation();return false;};LevelSelector.prototype.onShowLevelSelectMenu=function onShowLevelSelectMenu(){this.toggleContextMenu();};LevelSelector.prototype.hideSelectLevelMenu=function hideSelectLevelMenu(){this.$('.level_selector ul').hide();};LevelSelector.prototype.toggleContextMenu=function toggleContextMenu(){this.$('.level_selector ul').toggle();this.positionMenu(this.$('.level_selector ul'));};LevelSelector.prototype.positionMenu=function positionMenu($menu){if(!$menu.is(":visible"))return;var parent=this.$el.get(0);var buttonW=this.$el.width();var buttonH=this.$el.height();var buttonX=0;var buttonY=0;while(parent&&parent!==this.core.el){buttonX+=parent.offsetLeft;buttonY+=parent.offsetTop;parent=parent.offsetParent;}var maxHeight=buttonY;$menu.css('max-height',maxHeight+'px');var w=$menu.width();var h=$menu.height();var x=buttonX+(buttonW-w)/2;var y=buttonY-h;if(x+w>this.core.el.offsetWidth)x=this.core.el.offsetWidth-w;if(x<0)x=0;$menu.css('top',y-buttonY+'px');$menu.css('left',x-buttonX+'px');};LevelSelector.prototype.buttonElement=function buttonElement(){return this.$('.level_selector button');};LevelSelector.prototype.levelElement=function levelElement(id){return this.$('.level_selector ul a'+(!isNaN(id)?'[data-level-selector-select="'+id+'"]':'')).parent();};LevelSelector.prototype.getTitle=function getTitle(){return(this.core.options.levelSelectorConfig||{}).title;};LevelSelector.prototype.startLevelSwitch=function startLevelSwitch(){if(!this.assumeLevelChangesWillWork)this.buttonElement().addClass('changing');};LevelSelector.prototype.stopLevelSwitch=function stopLevelSwitch(){if(!this.assumeLevelChangesWillWork)this.buttonElement().removeClass('changing');};LevelSelector.prototype.updateText=function updateText(level){if(level===AUTO)this.buttonElement().text(this.activeLevel?'AUTO ('+this.activeLevel.label+')':'AUTO');else this.buttonElement().text(this.findLevelBy(level).label);};/**
	 ...
	 */LevelSelector.prototype.activeLevelChanged=function activeLevelChanged(info){this.activeLevel=this.findLevelBy(info.level);this.updateGUI();};/**
	 * Updates the GUI elements to match our current state.
	 */LevelSelector.prototype.updateGUI=function updateGUI(){this.levelElement().removeClass('current');if(this.activeLevel)this.levelElement(this.activeLevel.id).addClass('current');this.updateText(this.selectedLevelId);};(0,_createClass3.default)(LevelSelector,[{key:'supportedVersion',/**
	 * Clappr 0.4.x wants to know what versions this plugin is targeting.
	 */get:function get(){return{min:'0.4.0',max:'0.5.0'};}},{key:'assumeLevelChangesWillWork',get:function get(){return this.core.options.levelSelectorConfig&&this.core.options.levelSelectorConfig.assumeLevelChangesWillWork;}},{key:'name',get:function get(){return'level_selector';}},{key:'template',get:function get(){return(0,_player.template)(_levelSelector2.default);}},{key:'attributes',get:function get(){return{'class':this.name,'data-level-selector':''};}},{key:'events',get:function get(){return{'click [data-level-selector-select]':'onLevelSelect','click [data-level-selector-button]':'onShowLevelSelectMenu'};}},{key:'container',get:function get(){return this.core.activeContainer?this.core.activeContainer:this.core.mediaControl.container;}},{key:'playback',get:function get(){return this.core.activePlayback?this.core.activePlayback:this.core.getCurrentPlayback();}}],[{key:'version',get:function get(){return"0.4.3";}}]);return LevelSelector;}(_player.UICorePlugin);exports.default=LevelSelector;module.exports=exports['default'];
...
if(false) {
	...
	if(!content.locals) {
		module.hot.accept("!!../../node_modules/css-loader/index.js!../../node_modules/postcss-loader/lib/index.js!../../node_modules/sass-loader/lib/loader.js?includePaths[]=C:\\Users\\brpea\\Source\\Repos\\clappr-level-selector-plugin\\node_modules\\compass-mixins\\lib&includePaths[]=C:\\Users\\brpea\\Source\\Repos\\clappr-level-selector-plugin\\node_modules\\clappr\\src\\base\\scss&includePaths[]=C:\\Users\\brpea\\Source\\Repos\\clappr-level-selector-plugin\\src\\base\\scss!./style.scss", function() {
			var newContent = require("!!../../node_modules/css-loader/index.js!../../node_modules/postcss-loader/lib/index.js!../../node_modules/sass-loader/lib/loader.js?includePaths[]=C:\\Users\\brpea\\Source\\Repos\\clappr-level-selector-plugin\\node_modules\\compass-mixins\\lib&includePaths[]=C:\\Users\\brpea\\Source\\Repos\\clappr-level-selector-plugin\\node_modules\\clappr\\src\\base\\scss&includePaths[]=C:\\Users\\brpea\\Source\\Repos\\clappr-level-selector-plugin\\src\\base\\scss!./style.scss");
...
//# sourceMappingURL=level-selector.js.map